...
if p9sum is not None:
    ...
    if {"alpha_Dmax_mean","alpha_Dmax_lo","alpha_Dmax_hi"}.issubset(df.columns):
        a, lo, hi = df["alpha_Dmax_mean"].iloc[-1], df["alpha_Dmax_lo"].iloc[-1], df["alpha_Dmax_hi"].iloc[-1]
        alpha_ci_text = fmt_ci(a, lo, hi)
    if {"var_slope_mu","var_slope_lo","var_slope_hi"}.issubset(df.columns):
...
# Phase 3–7 quick inventory from known CSV names
known = [
...
# Build phase inventory table
table_lines: List[str] = []
if phase_rows:
    phase_rows.sort(key=lambda r: (r[0], r[1]))
    table_lines.append("\\section*{Phase inventory (auto)}")
    table_lines.append("\\begin{tabular}{cll}")
    table_lines.append("\\toprule")
    table_lines.append("Phase & Metric/Artifact & File " + "\\\\")
    table_lines.append("\\midrule")
    for ph, metric, fname in phase_rows:
        safe = fname.replace('_','\\_')
        table_lines.append(f"{ph} & {metric} & {safe} " + "\\\\")
...
content = (
    ...
    "\\begin{itemize}\n"
    f"\\item \\textbf{{Signed-$\\alpha$ at largest $\\D$}}: {alpha_ci_text} (0 lies inside CI).\n"
    f"\\item \\textbf{{Variance scaling}}: slope $\\beta$ for $\\log \\Var(\\Y)$ vs $\\log \\D$ = {var_slope_text}.\n"
...
OUTTEX.write_text(content.lstrip())
